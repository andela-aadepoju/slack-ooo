  ...
  announceOffline (users) {
    ...
    for (let x in users) {
      ...
      if (oooUser && oooUser.isOOO()) {
        ...
        const userName = user.profile.real_name || 'UNKNOWN_USER';
        const userMsg = oooUser.message ? `Here is ${userFirstName}'s personal message: ${oooUser.message}` : `${userFirstName} left no personal message`;
        found = true;
        retVal = `> ${userName} is out of the office.\n`;
        retVal += `> ${userMsg}\n`;
        retVal += `> You can call ${userFirstName} on ${userPhone}`;
      }
    }
